...
CLUE_LEMMAS = {
   "effect": {
      "increase", "decrease", "reduce", "enhance",
      "potentiate", "affect", "alter"
   },
   "mechanism": {
      "inhibit", "induce", "metabolize", "metabolism",
      "absorb", "bind", "eliminate", "excrete", "clearance"
   },
   "advise": {
      "avoid", "caution", "recommend", "monitor",
      "adjust", "contraindicate", "should", "must"
   },
   "int": {
      "interact", "interaction", "concomitant",
      "coadministration", "combination", "combine"
   }
}


def add_clue_features(feats, lemma, source):

   for cls, clues in CLUE_LEMMAS.items():
      has_clue = lemma in clues

      feats.add(f"has_{source}_{cls}_clue={has_clue}")

      if has_clue:
         feats.add(f"{source}_{cls}_clue={lemma}")
...
def extract_features(tree, entities, e1, e2) :
   ...
   if tkE1 is not None and tkE2 is not None:
      ...
      add_clue_features(feats, local_lemma, "local")
      ...
      add_clue_features(feats, lcs_lemma, "lcs")
